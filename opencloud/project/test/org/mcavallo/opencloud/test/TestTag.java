...
public class TestTag extends TestCase {
	...
	public void testTagConstructors() {
		...
		tag2 = new Tag( tag );
		assertEquals(name, tag2.getName());
		assertEquals(link, tag2.getLink());
		assertEquals(score, tag2.getScore());
		assertEquals(normScore, tag2.getNormScore());
		assertEquals(weight, tag2.getWeight());
		assertEquals(date, tag2.getDate());
	}
